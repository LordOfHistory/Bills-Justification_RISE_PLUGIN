...
class Juanmagestion extends Security_Controller {
    ...
    function index() {
        $data = array("title"=>"juanma", "uri_chunk"=>"exjus_juanma");
    ...
    function list_data() {    
        $this->have_access();
        $option = array("status" => "'w_for_juanma'");
        $list_data = $this->Expenses_model->get_details($option)->getResult();
    ...
    private function _make_row($data) {
        ...
        $buttons = "-";
        if ($data->status == "w_for_juanma"){
            $buttons = (
                modal_anchor(get_uri("exjus_juanma/accept_form"), "<i data-feather='check' class='icon-16'></i>", array("color"=>"blue","title" => app_lang("accept"), "data-post-id" => $data->id)).
                modal_anchor(get_uri("exjus_juanma/reject_form"), "<i data-feather='search' class='icon-16'></i>", array("class" => "reject", "title" =>  app_lang("reject").", ".app_lang("suggest_changes"), "data-post-id" => $data->id)).
                modal_anchor(get_uri("exjus_juanma/deny_form"), "<i data-feather='x' class='icon-16'></i>", array("class" => "delete", "title" => app_lang("deny"), "data-post-id" => $data->id))
            );
        }
        ...
        $row_data = array(
            ...
            $status,
            anchor(get_uri("exjus_juanma/expense_details/".$data->id), "<i data-feather='eye' class='icon-16'></i>", array("class"=>"details", "title"=>app_lang("view_details"))),
            $buttons,
        );
    ...
    function accept_form(){
        ...
        $view_data['submit_url'] = "exjus_juanma/accept";
    ...
    function accept(){
        ...
        if ($this->Expenses_model->updateDb($id,"status","w_for_payment")&& $this->Expenses_model->updateDb($id,"comments","")) {
    ...
    function reject_form(){
        ...
        $model_info = $this->Expenses_model->get_one($id); 
        $view_data['model_info'] = $model_info;
        $view_data['submit_url'] = "exjus_juanma/reject";
    ...
    function reject(){
        $this->have_access();
        $id = $this->request->getPost("id");
        $comments = "<b>".app_lang("juanma_comments").":</b> ".$this->request->getPost("comments");
        
        if ($this->Expenses_model->updateDb($id,"status","w_for_finnances") && $this->Expenses_model->updateDb($id,"comments",$comments)) {
            echo json_encode(array("success" => true, "data" => $this->_row_data($id), 'id' => $id, 'message' => app_lang('record_saved')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    ...
    function deny_form(){
        ...
        $model_info = $this->Expenses_model->get_one($id); 
        $view_data['model_info'] = $model_info;
        $view_data['submit_url'] = "exjus_juanma/deny";
    ...
    function expense_details($id=0){
        ...
        $view_data['model_info'] = $model_info;
        $view_data['user'] = $user;
        $view_data["prev_page"] = "exjus_juanma";
